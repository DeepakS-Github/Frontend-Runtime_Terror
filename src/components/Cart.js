...
function Cart(props) {
  ...
  // API Fetch
  let CartAPI = `${apiUrl}/api/showCartItem/${userData}`;
  ...
  useEffect(() => {
    ...
    axios
  .get(`${apiUrl}/api/showCartItem/${userData}`)
  .then((response) => {
    const mainArr = response.data.itemsId;
    const promises = [];

    for (let i = 0; i < mainArr.length; i++) {
      const nestedApiUrl = `${apiUrl}/api/getIdItemData/${mainArr[i]}`;
  ...
  const handleDelete = (toDelete) => {
    ...
    fetch(`${apiUrl}/api/deleteCartItem/${userData}/${toDelete}`, {
      method: "DELETE"
    })
      .then((response) => {
      ...
      itemsList.push(
        <li class="flex py-6">
          <div class="h-24 w-24 flex-shrink-0 overflow-hidden rounded-md border border-gray-200">
            <img
              src={`https://source.unsplash.com/random?${dataCart[i].image}&fit=crop&quality=low`}
              alt="Image"
